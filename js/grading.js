...
async function openSessionGrid(sessionId, kind, sessionNumber) {
  ...
  if (studentsErr || !students || students.length === 0) {
    contentArea.innerHTML = `<button class="btn-secondary" style="width:auto; padding:8px 16px; margin-bottom:16px;" onclick="openClass('${gradingClassId}', '${escapeAttr(gradingClassTitle)}')">← رجوع للفصل</button><div class="section-card"><div class="empty-state">ما فيه طلاب بهذا الفصل — أضفهم أولاً</div></div>`;
    return;
  }
...
async function loadReportBody() {
  ...
  bodyEl.innerHTML = `
    ...
    <div class="component-ring-grid">${results.map((r) => `<div class="component-mini-card"><div class="val">${r.avg}</div><div class="of">من ${r.target}</div><div class="lbl">${r.label}</div><div style="font-size:10px; color:var(--text-muted); margin-top:4px;">${r.count} ${r.key.includes("exam") ? "اختبار" : "حصة"} مسجلة</div></div>`).join("")}</div>`;
}
...
async function loadBehaviorNotes() {
  ...
  listEl.innerHTML = data.map((n) => `<div class="behavior-note ${n.note_type}"><div><div class="txt">${n.note_type === "positive" ? "🟢" : "🔴"} ${escapeHtml(n.note)}</div><div class="date">${new Date(n.created_at).toLocaleDateString("ar-SA")}</div></div><button class="icon-btn danger" onclick="deleteBehaviorNote('${n.id}')" title="حذف">✕</button></div>`).join("");
}
